...
export function ProductCard({
  ...
  const totalStock =
    relevantStocks?.reduce((acc, stock) => acc + stock.quantity, 0) || 0;
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    const bestStock = [...(relevantStocks || [])].sort(
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1 bg-white dark:bg-card">
        ...
        <p className="hidden lg:block text-sm text-slate-600 dark:text-slate-300 line-clamp-2 leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-300 absolute bottom-4 left-4 right-4 bg-white/90 dark:bg-card/90 backdrop-blur-sm p-3 rounded-xl border border-slate-100 dark:border-white/5 shadow-lg transform translate-y-2 group-hover:translate-y-0 z-20">
          {product.description || "Aucune description supplémentaire."}
        </p>

        <div className="mt-auto hidden">
          <div className="flex flex-col">
            <span className="text-2xl font-black tracking-tighter tabular-nums text-slate-900 dark:text-white group-hover:text-primary transition-colors">
              {formatCurrency(product.price)}
            </span>
