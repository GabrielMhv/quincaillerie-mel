...
export function NotificationBell({ userRole, boutiqueId }: { userRole: string, boutiqueId?: string }) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: { new: Notification }) => {
          ...
          if (userRole === "admin" || (boutiqueId && newNotif.type !== "low_stock" && newNotif.id)) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "new_order": return <Package className="h-4 w-4 text-primary" />;
      case "low_stock": return <AlertTriangle className="h-4 w-4 text-orange-500" />;
      case "transfer_request": return <CheckCircle2 className="h-4 w-4 text-emerald-500" />;
              ...
              >
                ...
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    <p className={cn("text-xs font-bold leading-none mb-1", !n.is_read && "text-primary")}>
                      {n.title}
                    </p>
                    {userRole === "admin" && n.boutique?.name && (
                      <Badge variant="outline" className="text-[9px] h-4 px-1 py-0 bg-primary/5 text-primary border-primary/20 mb-1">
                        {n.boutique.name}
                      </Badge>
                    )}
