...
export function TransferActions({ transferId, status, isSender }: TransferActionsProps) {
  const [loading, setLoading] = useState<string | null>(null);
  const supabase = createClient();
  const router = useRouter();

  if (status === 'completed' || status === 'cancelled' || status === 'rejected') return null;
    ...
    try {
      ...
      if (newStatus === "shipped") {
        ...
        if (transfer.product_id && transfer.quantity) {
          itemsToMove.push({ product_id: transfer.product_id, quantity: transfer.quantity });
        }
        if (transfer.stock_transfer_items?.length > 0) {
          transfer.stock_transfer_items.forEach((item: any) => {
            itemsToMove.push({ product_id: item.product_id, quantity: item.quantity });
          });
        }
        ...
        for (const item of itemsToMove) {
          ...
          if (!currentStock || currentStock.quantity < item.quantity) {
            throw new Error(`Stock insuffisant pour l'un des produits dans la boutique source.`);
          }
      ...
      return (
        <div className="flex gap-3 justify-end">
          <Button 
            ...
            disabled={!!loading}
          >
            {loading === 'rejected' ? <Loader2 className="h-3 w-3 animate-spin" /> : <XCircle className="h-4 w-4 mr-2" />}
            Refuser
          </Button>
          <Button 
            size="sm" 
            ...
            disabled={!!loading}
          >
            {loading === 'accepted' ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-4 w-4 mr-2" />}
            Accepter
          </Button>
        </div>
      );
    ...
    if (isSender) {
      return (
        <Button 
          ...
          disabled={!!loading}
        >
          {loading === 'shipped' ? <Loader2 className="h-4 w-4 animate-spin" /> : <MoveRight className="h-4 w-4" />}
          Expédier le stock
        </Button>
      );
    }
    return <Badge variant="outline" className="rounded-full bg-blue-500/5 text-blue-600 border-blue-500/20 px-4 py-1.5 text-[10px] font-black tracking-widest italic animate-pulse">PRÉPARATION...</Badge>;
  }
    ...
    if (!isSender) {
      return (
        <Button 
          ...
          disabled={!!loading}
        >
          {loading === 'completed' ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
          Confirmer réception
        </Button>
      );
    }
    return <Badge variant="outline" className="rounded-full bg-orange-500/5 text-orange-600 border-orange-500/20 px-4 py-1.5 text-[10px] font-black tracking-widest italic animate-pulse flex gap-2"><Truck className="h-3 w-3" /> EN ROUTE...</Badge>;
  }
