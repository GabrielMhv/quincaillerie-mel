...
import { ShoppingBag, User, Phone, MapPin, Calendar, Store, Info, Globe, PackageOpen, CreditCard } from "lucide-react";
...
export function OrderDetailsModal({ order, isOpen, onOpenChange }: OrderDetailsModalProps) {
  if (!order) return null;

  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[1100px] w-[95vw] rounded-[3rem] border-none shadow-3xl overflow-hidden p-0 bg-transparent gap-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-5 space-y-10">
                 ...
                 <div className="space-y-8 p-8 rounded-[2.5rem] bg-secondary/10 border border-border/50">
                    <div className="space-y-6">
                       ...
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <p className="font-black text-xl tracking-tight leading-none mb-1">{order.client_name}</p>
                       ...
                       {order.referred_employee_name && (
                         <div className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/10 flex items-center gap-3">
                            ...
                            <p className="text-[10px] font-bold text-indigo-600 italic">
                               Apporteur : <span className="tracking-tight not-italic">{order.referred_employee_name}</span>
                            </p>
                         </div>
                       )}
                    </div>
                 ...
                 <div className="relative group">
                    <div className="absolute -inset-1 bg-gradient-to-r from-primary via-indigo-600 to-primary rounded-[2.5rem] blur-xl opacity-20" />
                    <div className="relative flex flex-col justify-center gap-4 p-8 rounded-[2.5rem] bg-indigo-950 border border-white/10 text-white shadow-2xl overflow-hidden min-h-[140px]">
                       ...
                       <div className="relative z-10 w-full overflow-hidden text-right">
                          <span className="text-3xl sm:text-4xl font-black tabular-nums tracking-tighter antialiased drop-shadow-lg break-words">
              ...
              <div className="lg:col-span-7 space-y-6">
                 <div className="flex justify-between items-end border-b border-border/50 pb-4">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground/40 italic">Panier détaillé</p>
                 </div>
                 
                 <ScrollArea className="h-[430px] pr-4">
                   <div className="space-y-3">
                      {order.order_items?.map((item: any, idx: number) => (
                        <div key={idx} className="group relative flex justify-between items-center p-4 rounded-2xl bg-secondary/5 hover:bg-white dark:hover:bg-card hover:shadow-lg transition-all border border-transparent hover:border-border/50">
                          <div className="flex items-center gap-4">
                             <div className="h-10 w-10 rounded-xl bg-primary/5 flex items-center justify-center text-primary/40 font-black text-xs">
                                {idx + 1}
                             </div>
                             <div className="space-y-1">
                               <p className="font-black text-sm tracking-tight group-hover:text-primary transition-colors">{item.product?.name || "Produit supprimé"}</p>
                               <div className="flex items-center gap-3">
                                  <Badge variant="secondary" className="rounded-lg px-2 py-0 text-[8px] font-bold tracking-tight bg-muted/40 border-none">
                                     Qté: {item.quantity}
                                  </Badge>
                          ...
                          <p className="font-black text-base tabular-nums tracking-tighter">{formatCurrency(item.quantity * item.price)}</p>
                        </div>
                      ))}
                      {(!order.order_items || order.order_items.length === 0) && (
                        <div className="h-full flex flex-col items-center justify-center py-20 opacity-20">
                           <PackageOpen className="h-12 w-12 mb-4" />
                           <p className="text-sm font-black tracking-tight italic">Aucun article enregistré</p>
                        </div>
                      )}
                   </div>
                 </ScrollArea>
