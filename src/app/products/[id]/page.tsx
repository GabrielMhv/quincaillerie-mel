          ...
          <div className="grid gap-12 lg:gap-20 lg:grid-cols-2 lg:items-start">
            ...
            <div className="flex flex-col gap-8 md:gap-12 animate-in fade-in slide-in-from-right-12 duration-1000 delay-200 text-center lg:text-left">
              ...
              <div className="space-y-4 max-w-2xl mx-auto lg:mx-0">
                <h3 className="text-[10px] font-black tracking-[0.2em] text-slate-400">DESCRIPTION DÉTAILLÉE</h3>
                <p className="text-base md:text-xl text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                  {typedProduct.description || "Aucune description détaillée n'est disponible pour ce produit d'exception."}
                </p>
              </div>
              ...
              {typedProduct.stocks && typedProduct.stocks.length > 0 && (
                <div className="space-y-6 pt-8 border-t border-slate-200 dark:border-white/5 mx-auto lg:mx-0 w-full max-w-md">
                  <h3 className="text-[10px] font-black tracking-[0.2em] text-slate-400 text-center lg:text-left">POINTS DE VENTE ACTIFS</h3>
                  <div className="grid gap-4">
                    {typedProduct.stocks.map((stock: any) => (
                      <div key={stock.id} className="flex items-center justify-between p-5 rounded-2xl md:rounded-3xl border border-white/20 dark:border-white/5 bg-white/40 dark:bg-white/5 backdrop-blur-xl shadow-sm group hover:scale-[1.02] transition-all">
                        <div className="space-y-1 text-left">
                          <span className="text-[9px] font-black tracking-widest text-primary opacity-60">BOUTIQUE</span>
                          <p className="text-base font-black tracking-tight leading-none text-slate-800 dark:text-white uppercase">{stock.boutique?.name}</p>
                        </div>
            ...
            <div className="flex flex-col gap-4 border-t pt-8">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 pt-12 border-t border-slate-200 dark:border-white/5">
                {[
                  { icon: Truck, title: "LIVRAISON TOGO", desc: "Sous 24-48h à Lomé" },
                  { icon: ShieldCheck, title: "GARANTIE PRO", desc: "Qualité certifiée" },
                  { icon: RefreshCcw, title: "RETOURS FACILES", desc: "Service client 7j/7" },
                ].map((item, i) => (
                  <div key={i} className="flex flex-col items-center text-center p-6 rounded-3xl bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/5 shadow-sm group hover:-translate-y-1 transition-all">
                    ...
                    <span className="text-[10px] font-black tracking-widest text-slate-900 dark:text-white mb-2">{item.title}</span>
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">{item.desc}</span>
                  </div>
                ))}
              </div>
            </div>
