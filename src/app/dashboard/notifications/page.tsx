...
export default function NotificationsPage() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: { new: Notification }) => {
          ...
          const isRelevant = userRole === "admin" || (boutiqueId && newNotif.boutique_id === boutiqueId);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "new_order": return <Package className="h-5 w-5 text-primary" />;
      case "low_stock": return <AlertTriangle className="h-5 w-5 text-orange-500" />;
      case "transfer_request": return <CheckCircle2 className="h-5 w-5 text-emerald-500" />;
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-5xl font-black tracking-tighter leading-tight">
              Centre de <span className="text-gradient leading-relaxed">Notifications</span>
            </h1>
          </div>
          ...
          <div className="flex bg-muted/50 p-1.5 rounded-2xl border border-border/50">
            <button
               ...
               className={cn(
                 ...
                 filter === "all" ? "bg-white text-primary shadow-sm ring-1 ring-border/20" : "text-muted-foreground hover:text-foreground"
               )}
            >
              TOUTES
            </button>
            <button
               ...
               className={cn(
                 "px-6 py-2.5 rounded-xl text-xs font-black tracking-widest transition-all",
                 filter === "unread" ? "bg-white text-primary shadow-sm ring-1 ring-border/20" : "text-muted-foreground hover:text-foreground"
               )}
            >
              NON LUES
            </button>
          ...
          {loading ? (
            <div className="flex h-[500px] items-center justify-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-xs font-black tracking-widest text-muted-foreground/40 italic uppercase">Syncronisation du flux...</p>
              </div>
            </div>
          ) : filteredNotifications.length === 0 ? (
            <div className="flex h-[500px] flex-col items-center justify-center text-center p-12">
              ...
              <p className="text-xl font-black tracking-tight mb-2">Historique vide</p>
              <p className="text-sm text-muted-foreground max-w-xs font-medium italic">
                {search ? "Aucune notification ne correspond à votre recherche." : "Vous n'avez pas encore reçu de notifications."}
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div className="mt-1 relative shrink-0">
                      <div className={cn(
                        ...
                        n.is_read ? "bg-muted/30 border-border/50" : "bg-primary/10 border-primary/20 shadow-[0_0_20px_rgba(var(--primary),0.1)]"
                      )}>
                        {getIcon(n.type)}
                      </div>
                      {!n.is_read && (
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="flex flex-wrap items-center gap-3">
                        <h4 className={cn(
                          ...
                          n.is_read ? "font-bold text-foreground/70" : "font-black text-foreground"
                        )}>
                          {n.title}
                        </h4>
                        {!n.is_read && (
                        ...
                        {n.boutique?.name && (
                           <Badge variant="outline" className="text-[10px] h-5 rounded-lg bg-muted text-muted-foreground border-border/50 font-bold">
                             {n.boutique.name}
                           </Badge>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-6 text-[11px] font-bold tracking-tight text-muted-foreground/40">
                         <div className="flex items-center gap-2">
                           ...
                           {format(new Date(n.created_at), "eeee d MMMM 'à' HH:mm", { locale: fr })}
                         </div>
                         <div className="flex items-center gap-2">
