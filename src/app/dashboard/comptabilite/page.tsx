...
export default async function AccountingPage(props: {
  ...
  if (role !== "admin" && role !== "manager") {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center p-12 text-center bg-card/50 backdrop-blur-xl rounded-[4rem] border border-dashed border-primary/20">
        ...
        <h2 className="text-3xl font-black tracking-tighter italic">Accès Restreint</h2>
  ...
  const currentMonthOrders = validOrders.filter(o => new Date(o.created_at) >= startOfMonthDate);
  const lastMonthOrders = validOrders.filter(o => {
    const d = new Date(o.created_at);
    return d >= startOfLastMonthDate && d <= endOfLastMonthDate;
  });

  const currentMonthRevenue = currentMonthOrders.reduce((acc, o) => acc + (o.total_amount || 0), 0);
  const lastMonthRevenue = lastMonthOrders.reduce((acc, o) => acc + (o.total_amount || 0), 0);
  ...
  const estimatedExpenses = totalRevenue * 0.65; // Placeholder logic
  const netMargin = totalRevenue - estimatedExpenses;
    ...
    <div className="space-y-10 pb-20 max-w-7xl mx-auto animate-in fade-in duration-700">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 px-4 sm:px-0 mt-8">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Badge variant="outline" className="rounded-full bg-primary/5 text-primary border-primary/20 px-4 py-1 font-bold text-[10px] tracking-widest italic">
              Finance & Performance
            </Badge>
          </div>
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-tight italic">
            Compta<span className="text-primary tracking-[-0.2em] ml-2">bilité</span>
          </h1>
        ...
        <div className="flex items-center gap-4">
          <ExportButtons data={validOrders.map(o => ({
      ...
      <section className="p-6 md:p-8 rounded-[3rem] bg-card/40 backdrop-blur-xl border border-border/50 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8 -translate-y-4">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h3 className="text-xl font-black tracking-tight italic">Filtres de Trésorerie</h3>
            <p className="text-sm text-muted-foreground font-medium italic tracking-tighter">Affinage des données temporelles</p>
          </div>
        </div>
        <DashboardFilters />
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-4 sm:px-0">
         ...
         <Card className="rounded-[2.5rem] shadow-2xl bg-linear-to-br from-indigo-600 to-indigo-900 text-white overflow-hidden group border-none">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-center mb-4">
              ...
              <Badge className="bg-white/20 border-none text-white font-bold italic">Mensuel</Badge>
            </div>
            <CardTitle className="text-sm font-bold tracking-widest opacity-70 italic">Chiffre d&apos;Affaires</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className={cn(
              ...
              revenueGrowth >= 0 ? "text-emerald-300" : "text-rose-300"
            )}>
        ...
        <Card className="rounded-[2.5rem] bg-card/40 backdrop-blur-xl border border-border/50 shadow-2xl overflow-hidden relative">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-sm font-bold tracking-widest text-muted-foreground italic">Total Cumulé</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-black tracking-tighter italic">
              {formatCurrency(totalRevenue)}
            </div>
            <p className="text-xs text-muted-foreground mt-2 font-medium italic tracking-tighter">Depuis le début des opérations</p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-[2.5rem] bg-card/40 backdrop-blur-xl border border-border/50 shadow-2xl overflow-hidden relative">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-sm font-bold tracking-widest text-muted-foreground italic">Charges Estimées</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-black tracking-tighter italic text-rose-500">
              {formatCurrency(estimatedExpenses)}
            </div>
            <p className="text-xs text-muted-foreground mt-2 font-medium italic tracking-tighter">Estimation basique (65%)</p>
          </CardContent>
        </Card>
         ...
         <Card className="rounded-[2.5rem] bg-card/40 backdrop-blur-xl border border-border/50 shadow-2xl overflow-hidden relative">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-sm font-bold tracking-widest text-muted-foreground italic">Marge Nette</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-black tracking-tighter italic text-emerald-500">
              {formatCurrency(netMargin)}
            </div>
            <p className="text-xs text-muted-foreground mt-2 font-medium italic tracking-tighter">Profit brut avant taxes</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-8 px-4 sm:px-0">
        <div className="flex justify-center">
            <TabsList className="bg-card/40 backdrop-blur-xl border border-border/50 p-2 rounded-full h-16 shadow-2xl">
                <TabsTrigger value="overview" className="rounded-full px-8 h-full font-bold text-sm tracking-widest italic data-[state=active]:bg-primary data-[state=active]:text-white transition-all">
                    Vue d&apos;ensemble
                </TabsTrigger>
                <TabsTrigger value="transactions" className="rounded-full px-8 h-full font-bold text-sm tracking-widest italic data-[state=active]:bg-primary data-[state=active]:text-white transition-all">
                    Journal des Ventes
                </TabsTrigger>
                <TabsTrigger value="tax" className="rounded-full px-8 h-full font-bold text-sm tracking-widest italic data-[state=active]:bg-primary data-[state=active]:text-white transition-all">
                    Fiscalité &amp; Rapports
                </TabsTrigger>
            </TabsList>
        </div>
                ...
                <Card className="rounded-[3rem] border-none shadow-2xl bg-card/40 backdrop-blur-xl p-8">
                    <CardHeader className="px-0 pt-0">
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <CardTitle className="text-2xl font-black tracking-tighter italic">Flux Mensuel</CardTitle>
                        </div>
                    </CardHeader>
                    <div className="h-75 w-full flex items-center justify-center border border-dashed border-border/40 rounded-4xl bg-muted/20">
                        <div className="text-center">
                            ...
                            <p className="text-sm font-bold text-muted-foreground italic tracking-widest">Graphique de tendance en cours d&apos;intégration</p>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="rounded-[3rem] border-none shadow-2xl bg-card/40 backdrop-blur-xl p-8">
                    <CardHeader className="px-0 pt-0">
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <CardTitle className="text-2xl font-black tracking-tighter italic">Répartition par Boutique</CardTitle>
                        </div>
                    </CardHeader>
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between p-6 rounded-3xl bg-secondary/20 border border-border/50">
                            <div>
                                <p className="text-xs font-bold text-primary italic mb-1">Boutique A</p>
                                <p className="text-xl font-black tracking-tighter italic">{formatCurrency(totalRevenue * 0.6)}</p>
                            </div>
                            <Badge className="bg-emerald-500/10 text-emerald-500 border-none">60%</Badge>
                        </div>
                        <div className="flex items-center justify-between p-6 rounded-3xl bg-secondary/20 border border-border/50">
                             <div>
                                <p className="text-xs font-bold text-indigo-500 italic mb-1">Boutique B</p>
                                <p className="text-xl font-black tracking-tighter italic">{formatCurrency(totalRevenue * 0.4)}</p>
                            </div>
                            <Badge className="bg-indigo-500/10 text-indigo-500 border-none">40%</Badge>
                        </div>
                    </div>
                </Card>
            </div>
        ...
        <TabsContent value="transactions">
             <Card className="rounded-[3rem] border-none shadow-2xl bg-card/40 backdrop-blur-xl overflow-hidden">
                <CardHeader className="p-8 pb-0">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-4">
                            ...
                            <CardTitle className="text-2xl font-black tracking-tighter italic">Grand Livre des Ventes</CardTitle>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-border/50 bg-secondary/30">
                                <th className="px-8 py-5 text-xs font-bold tracking-widest text-muted-foreground italic">ID</th>
                                <th className="px-8 py-5 text-xs font-bold tracking-widest text-muted-foreground italic">Date</th>
                                <th className="px-8 py-5 text-xs font-bold tracking-widest text-muted-foreground italic">Client</th>
                                <th className="px-8 py-5 text-xs font-bold tracking-widest text-muted-foreground italic">Boutique</th>
                                <th className="px-8 py-5 text-right text-xs font-bold tracking-widest text-muted-foreground italic">Montant</th>
                                <th className="px-8 py-5 text-center text-xs font-bold tracking-widest text-muted-foreground italic">Statut</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-border/50">
                            {validOrders.slice(0, 10).map((order) => (
                                <tr key={order.id} className="hover:bg-primary/5 transition-colors group">
                                    <td className="px-8 py-5 text-xs font-bold text-muted-foreground">#{order.id.substring(0, 8)}</td>
                                    <td className="px-8 py-5 text-sm font-bold italic">{format(new Date(order.created_at), 'dd MMM yyyy HH:mm', { locale: fr })}</td>
                                    <td className="px-8 py-5 text-sm font-bold italic">{order.customer_name || 'Client Anonyme'}</td>
                                    <td className="px-8 py-5">
                                        <Badge variant="outline" className="font-bold italic text-[9px] border-primary/20 text-primary">
                                            {order.boutique?.name || 'Globale'}
                                        </Badge>
                                    </td>
                                    <td className="px-8 py-5 text-right text-sm font-black italic">{formatCurrency(order.total_amount)}</td>
                                    <td className="px-8 py-5 text-center">
                                         <Badge className={cn(
                                             "font-bold italic text-[9px]",
                                             order.status === 'completed' ? "bg-emerald-500/20 text-emerald-500 border-none" : "bg-orange-500/20 text-orange-500 border-none"
                                         )}>
                                             {order.status}
                                         </Badge>
                                    </td>
                ...
                <div className="p-8 text-center border-t border-border/50">
                    <p className="text-sm font-bold text-muted-foreground italic tracking-widest cursor-pointer hover:text-primary transition-colors underline-offset-4 hover:underline">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <Card className="rounded-[3rem] shadow-2xl bg-card/40 backdrop-blur-xl p-10 border-2 border-dashed border-primary/20">
                    <div className="text-center space-y-6">
                        ...
                        <div>
                            <h3 className="text-2xl font-black tracking-tighter italic">Rapport Fiscal Annuel</h3>
                            <p className="text-sm text-muted-foreground font-medium italic mt-2 leading-relaxed">
                                Générez un document consolidé pour vos déclarations d&apos;impôts et bilans comptables de l&apos;exercice en cours.
                            </p>
                        </div>
                ...
                <Card className="rounded-[3rem] shadow-2xl bg-card/40 backdrop-blur-xl p-10 border-2 border-dashed border-emerald-500/20">
                    <div className="text-center space-y-6">
                        ...
                        <div>
                            <h3 className="text-2xl font-black tracking-tighter italic">Relevé de TVA</h3>
                            <p className="text-sm text-muted-foreground font-medium italic mt-2 leading-relaxed">
                                Calcul automatique de la TVA collectée sur l&apos;ensemble des transactions par période fiscale.
                            </p>
                        </div>
