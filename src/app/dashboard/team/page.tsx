...
import { Users, Shield, Sparkles, Mail, Building2, UserCircle } from "lucide-react";
  ...
  if (profile?.role !== "admin") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-xl font-black tracking-tighter italic">Périmètre Administrateur</p>
        <p className="text-sm text-muted-foreground font-medium">L&apos;accès à la gestion du personnel est restreint.</p>
      </div>
    );
  }

  // Fetch Boutiques
  const { data: boutiques } = await supabase.from("boutiques").select("*").order("name");
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          <h1 className="text-5xl font-black tracking-tighter leading-tight">
             Gestion de l&apos;<span className="text-gradient leading-relaxed">Équipe</span>
          </h1>
      ...
      <section className="rounded-[4rem] border border-border/50 bg-card/80 backdrop-blur-xl shadow-premium overflow-hidden">
        <div className="p-10 border-b border-border/50 bg-muted/30 flex justify-between items-center">
           ...
           <div className="px-5 py-2 rounded-2xl bg-primary/10 border border-primary/20 flex items-center gap-2">
              ...
              <span className="text-xs font-black tracking-widest text-primary italic">Structure Active</span>
           </div>
        </div>
        ...
        <div className="max-w-full overflow-x-auto custom-scrollbar">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/20">
              {users?.map((u) => (
                <tr key={u.id} className="group hover:bg-primary/[0.02] transition-colors h-28">
                  <td className="px-10">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <p className="text-lg font-black tracking-tight group-hover:text-primary transition-colors">
                            {u.name} {u.id === user.id && <span className="text-xs font-medium text-primary/60 ml-1">(Vous)</span>}
                          </p>
                       </div>
                    </div>
                  </td>
                  ...
                  <td className="px-10">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="font-bold text-muted-foreground">{u.boutique?.name || "Siège Social"}</span>
                    </div>
                  </td>
                  <td className="px-10 text-right">
                    <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0 transition-all duration-500">
                      <UserFormModal boutiques={boutiques || []} userToEdit={u} />
                    </div>
                  </td>
                </tr>
              ))}
                ...
                <tr className="h-80">
                  <td colSpan={5} className="text-center">
                    <div className="flex flex-col items-center justify-center space-y-5 opacity-20">
                       ...
                       <p className="text-sm font-black tracking-widest italic opacity-40">Aucun collaborateur trouvé</p>
                    </div>
                  </td>
                </tr>
              )}
