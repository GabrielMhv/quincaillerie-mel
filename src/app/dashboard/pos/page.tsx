...
export default async function POSPage(props: {
  ...
  const finalBoutiqueId = (profile?.role === "admin" && boutiqueSwitcherId) 
    ? boutiqueSwitcherId 
    : profile?.boutique_id;
  ...
  let boutiqueName = "";
  if (finalBoutiqueId) {
    const { data: bData } = await supabase.from("boutiques").select("name").eq("id", finalBoutiqueId).single();
      ...
      <div className="flex h-[70vh] items-center justify-center p-12 bg-card/50 backdrop-blur-xl rounded-[4rem] border border-dashed border-primary/20 animate-in fade-in duration-1000">
        <div className="text-center max-w-lg space-y-6">
          ...
          <div className="space-y-2">
            <h2 className="text-3xl font-black tracking-tighter">Sélection requise</h2>
            <p className="text-muted-foreground font-medium leading-relaxed italic">
              Pour activer le terminal de vente, veuillez sélectionner un point de vente dans le menu supérieur.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full flex flex-col space-y-8 animate-in fade-in duration-1000">
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-5xl font-black tracking-tighter leading-tight">
             Terminal de <span className="text-gradient leading-relaxed">Vente</span>
          </h1>
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-2 px-4 py-1.5 rounded-2xl bg-secondary/50 border border-border/50 shadow-sm">
                ...
                <span className="text-sm font-black tracking-tight">{boutiqueName}</span>
             </div>
        ...
        <div className="hidden xl:flex items-center gap-4 p-4 rounded-3xl bg-card border border-border/50 shadow-sm">
           ...
           <div>
              <p className="text-[10px] font-black text-muted-foreground/40 tracking-widest italic">Prêt pour l&apos;encaissement</p>
      ...
      <div className="flex-1 min-h-[600px] rounded-[3rem] overflow-hidden border border-border/50 shadow-premium bg-card/40 backdrop-blur-md">
