...
export default async function DashboardStocksPage(props: {
  ...
  if (!profile || (profile.role !== "admin" && profile.role !== "manager" && profile.role !== "employee")) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        <div className="h-16 w-16 rounded-[2rem] bg-rose-500/10 flex items-center justify-center text-rose-600">
           <AlertTriangle className="h-8 w-8" />
        </div>
        <p className="text-xl font-black tracking-tighter italic">Accès hautement sécurisé</p>
        <p className="text-sm text-muted-foreground font-medium">Réservé au personnel autorisé de la direction.</p>
      </div>
    );
  }
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          <h1 className="text-5xl font-black tracking-tighter leading-tight">
             Contrôle des <span className="text-gradient leading-relaxed">Stocks</span>
          </h1>
      ...
      <section className="rounded-[4rem] border border-border/50 bg-card/80 backdrop-blur-xl shadow-premium overflow-hidden">
        <div className="p-10 border-b border-border/50 bg-muted/30 flex justify-between items-center">
           ...
           <div className="px-5 py-2 rounded-2xl bg-primary/10 border border-primary/20 flex items-center gap-2">
              ...
              <span className="text-xs font-black tracking-widest text-primary italic">Sync. Temps-Réel</span>
           </div>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="text-[10px] font-black text-muted-foreground/50 tracking-[0.2em] border-b border-border/30 h-16 bg-muted/10">
                <th className="px-10 text-left sticky left-0 bg-muted/10 backdrop-blur-md z-20 border-r border-border/30 w-72">Produit & Unité</th>
                {boutiques?.map((boutique) => (
                  <th key={boutique.id} className="px-10 text-center border-r border-border/30 last:border-r-0 min-w-[220px]">
                    {boutique.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-border/20">
              {products?.map((product) => (
                <tr key={product.id} className="group hover:bg-primary/[0.02] transition-all h-32 border-none">
                  ...
                  {boutiques?.map((boutique) => {
                    const stockRecord = product.stocks?.find((s: any) => s.boutique_id === boutique.id);
                    const currentStock = stockRecord?.quantity || 0;
                    const isLow = currentStock <= (product.min_stock_alert || 0);

                    return (
                      <td key={`${product.id}-${boutique.id}`} className="px-10 text-center border-r border-border/30 last:border-r-0">
                        <div className="flex flex-col items-center gap-4">
                          {isLow && (
                            <Badge variant="outline" className="rounded-full px-3 py-0.5 text-[9px] font-black tracking-widest bg-rose-500/10 text-rose-600 border-rose-500/20 animate-pulse shadow-[0_0_15px_-5px_rgba(244,63,94,0.4)]">
                              Stock Critique
                            </Badge>
                          )}
                          <StockEditor
                            ...
                            disabled={
                              ...
                              (profile.role === "manager" && boutique.id !== profile.boutique_id)
                            }
                          />
                        </div>
                      </td>
              ...
              {(!products || products.length === 0) && (
                <tr className="h-80">
                  <td colSpan={(boutiques?.length || 0) + 1} className="text-center">
                    <div className="flex flex-col items-center justify-center space-y-5 opacity-20">
                       <Boxes className="h-16 w-16" />
                       <p className="text-lg font-black tracking-widest text-center">Aucun produit au catalogue</p>
                    </div>
                  </td>
                </tr>
              )}
