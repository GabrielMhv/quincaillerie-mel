    ...
    switch (status) {
      case "pending":
        return (
          <Badge
            variant="outline"
            className="bg-orange-500/10 text-orange-600 border-orange-500/20 rounded-full px-4 py-1 text-[10px] font-black tracking-widest gap-1.5 animation-pulse"
          >
      ...
      case "accepted":
        return (
          <Badge className="bg-blue-500/10 text-blue-600 border-blue-500/20 rounded-full px-4 py-1 text-[10px] font-black tracking-widest gap-1.5">
            <CheckCircle2 className="h-3 w-3" /> Accepté
          </Badge>
        );
      case "shipped":
        return (
          <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20 rounded-full px-4 py-1 text-[10px] font-black tracking-widest gap-1.5 animate-pulse">
            <Truck className="h-3 w-3" /> Expédié
          </Badge>
        );
      case "completed":
        return (
          <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 rounded-full px-4 py-1 text-[10px] font-black tracking-widest gap-1.5">
      ...
      case "rejected":
        return (
          <Badge
            variant="destructive"
            className="rounded-full px-4 py-1 text-[10px] font-black tracking-widest gap-1.5"
          >
      ...
      case "cancelled":
        return (
          <Badge
            variant="outline"
            className="text-muted-foreground/60 border-muted-foreground/20 rounded-full px-4 py-1 text-[10px] font-black tracking-widest gap-1.5"
          >
      ...
      default:
        return (
          <Badge
            variant="outline"
            className="rounded-full px-4 py-1 text-[10px] font-black tracking-widest"
          >
            {status}
          </Badge>
        );
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        {boutiqueId && (
          <Link href="/dashboard/stocks/request">
            <Button className="h-16 px-10 rounded-4xl bg-primary text-primary-foreground font-black tracking-tighter text-lg hover:scale-105 transition-all shadow-lg flex gap-3">
                      ...
                      <div className="flex flex-col">
                        {isIncoming ? (
                          ...
                          <div className="flex items-center gap-2 text-muted-foreground/60 font-black tracking-tighter italic text-base">
                            <ArrowRightLeft className="h-5 w-5" /> Transfert
                            Réseau
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-lg font-black tracking-tighter">
                            {tx.items_count[0]?.count || 0}
                          </span>
                          <span className="text-[10px] font-bold text-muted-foreground/60 tracking-widest">
                            ARTICLES
                          </span>
                  ...
                  <td colSpan={6} className="text-center">
                    <div className="flex flex-col items-center justify-center space-y-5 opacity-20">
                      ...
                      <p className="text-lg font-black tracking-widest italic">
                        Aucun mouvement logistique répertorié
                      </p>
                    </div>
                  </td>
                </tr>
