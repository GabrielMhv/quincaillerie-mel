...
import { Users, Search, TrendingUp, Sparkles, UserCircle, Phone, MapPin, ShoppingBag, ArrowRight } from "lucide-react";
        ...
        <div className="max-w-full overflow-x-auto custom-scrollbar">
          <Table>
            ...
            <TableBody className="divide-y divide-border/20">
              {clients.map((c, i) => (
                <TableRow key={i} className="group hover:bg-primary/[0.02] transition-colors h-28 border-none">
                  <TableCell className="px-10">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <p className="text-lg font-black tracking-tight group-hover:text-primary transition-colors">{c.name}</p>
                          <p className="text-[10px] font-black text-muted-foreground/40 tracking-widest uppercase italic">Client Régulier</p>
                       </div>
                    </div>
                  </TableCell>
                  <TableCell className="px-10">
                    <div className="space-y-1.5 opacity-70 group-hover:opacity-100 transition-opacity">
                       <div className="flex items-center gap-2 text-sm font-bold tracking-tight">
                          <Phone className="h-3.5 w-3.5 text-primary/40" /> {c.phone}
                       </div>
                       <div className="flex items-center gap-2 text-xs font-medium italic">
                          <MapPin className="h-3.5 w-3.5 text-primary/40" /> {c.address}
                       </div>
                    </div>
                  </TableCell>
                  <TableCell className="px-10 text-center">
                     <div className="inline-flex flex-col items-center">
                        <Badge variant="outline" className="rounded-full px-4 py-1.5 flex items-center gap-2 bg-indigo-500/5 text-indigo-500 border-indigo-500/10 font-black text-[10px] tracking-widest leading-none">
                           <ShoppingBag className="h-3.5 w-3.5" /> {c.orderCount} Commandes
                        </Badge>
                     </div>
                  </TableCell>
                ...
                <TableRow className="h-80">
                  <TableCell colSpan={4} className="text-center">
                    <div className="flex flex-col items-center justify-center space-y-5 opacity-20">
                       ...
                       <p className="text-lg font-black tracking-widest italic">Aucun client répertorié</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
