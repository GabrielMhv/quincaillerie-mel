...
import { BarChart3, Users, Star, Trophy, TrendingUp, Zap, Target, Medal, Sparkles } from "lucide-react";
...
export default async function StatsPage(props: {
  ...
  let referralsQuery = supabase.from("employee_referrals").select(`
    *,
  ...
  const totalRevenue = rankedEmployees.reduce((sum, emp) => sum + emp.revenue, 0);
  const totalReferrals = rankedEmployees.reduce((sum, emp) => sum + emp.referralsCount, 0);
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          <h1 className="text-5xl font-black tracking-tighter leading-tight">
             Performances <span className="text-gradient leading-relaxed">Élite</span>
          </h1>
          <p className="text-lg text-muted-foreground font-medium italic">
            {isGlobalScope ? "Synthèse analytique du réseau de distribution" : "Analyse d&apos;efficacité de l&apos;unité locale"}
          </p>
        </div>
        {!isGlobalScope && (
      ...
      <div className="grid gap-10 md:grid-cols-3">
        <div className="rounded-[3.5rem] border border-border/50 bg-card/80 backdrop-blur-xl shadow-premium p-10 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-6">
             ...
             <div className="space-y-1">
                <p className="text-[10px] font-black tracking-widest text-muted-foreground/60 italic uppercase mb-1">Volume de Production (Équipe)</p>
                <p className="text-4xl font-black tracking-tighter text-gradient">{formatCurrency(totalRevenue)}</p>
             </div>
          </div>
        </div>
        ...
        <div className="rounded-[3.5rem] border border-border/50 bg-card/80 backdrop-blur-xl shadow-premium p-10 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-6">
             ...
             <div className="space-y-1">
                <p className="text-[10px] font-black tracking-widest text-muted-foreground/60 italic uppercase mb-1">Impact des Recommandations</p>
                <p className="text-5xl font-black tracking-tighter text-indigo-600 leading-none">{totalReferrals}</p>
             </div>
          </div>
        </div>
        ...
        <div className="rounded-[3.5rem] border border-border/50 bg-card/80 backdrop-blur-xl shadow-premium p-10 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-6">
             ...
             <div className="space-y-1">
                <p className="text-[10px] font-black tracking-widest text-muted-foreground/60 italic uppercase mb-1">Capital Humain Actif</p>
                <p className="text-5xl font-black tracking-tighter text-orange-600 leading-none">{rankedEmployees.length}</p>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[4rem] border border-border/50 bg-card/80 backdrop-blur-xl shadow-premium overflow-hidden">
        <div className="p-10 border-b border-border/50 bg-muted/30 flex justify-between items-center">
           <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground font-medium italic uppercase tracking-widest opacity-40">Intelligence de performance individuelle</p>
           </div>
           <div className="px-6 py-2 rounded-2xl bg-primary/10 border border-primary/20 flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-xs font-black tracking-[0.2em] text-primary italic">RANKING 2.0</span>
           </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/20">
              {rankedEmployees.map((emp, index) => (
                <tr key={emp.id} className="group hover:bg-primary/[0.02] transition-colors h-28">
                  <td className="px-10">
                    <div className={cn(
                      "h-12 w-12 rounded-[1rem] font-black text-xl flex items-center justify-center transition-transform group-hover:scale-110",
                      index === 0 ? "bg-yellow-500/20 text-yellow-600 border border-yellow-500/20" : 
                      index === 1 ? "bg-slate-400/20 text-slate-500 border border-slate-400/20" : 
                      index === 2 ? "bg-orange-500/20 text-orange-600 border border-orange-500/20" : 
                  ...
                  <td className="px-10">
                    <div className="flex flex-col">
                       <span className="font-black text-lg tracking-tighter leading-none">{emp.name}</span>
                       <span className="text-[10px] font-bold text-muted-foreground/60 italic tracking-widest mt-1 opacity-60">MEMBRE CERTIFIÉ</span>
                    </div>
                  </td>
                  <td className="px-10 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-muted/50 border border-border/50 text-xs font-black">
                       ...
                       {emp.salesCount} <span className="text-[8px] opacity-40 ml-1">TRANSACTIONS</span>
                    </div>
                  </td>
                  <td className="px-10 text-center">
                     {emp.referralsCount > 0 ? (
                       ...
                       <span className="text-muted-foreground/20 italic font-bold">Neutre</span>
                     )}
                  </td>
                ...
                <tr>
                  <td colSpan={5} className="h-80 text-center">
                     <div className="flex flex-col items-center justify-center space-y-6 opacity-10">
                        <Users className="h-20 w-20" />
                        <p className="text-xl font-black tracking-widest italic">Signal de performance inexistant</p>
                     </div>
                  </td>
                </tr>
              )}
